...
\begin{longtable}{|>{\centering\arraybackslash\small}p{2.2cm}|>{\centering\arraybackslash\small}p{0.9cm}|>{\small}p{7.4cm}|>{\centering\arraybackslash\small}p{1.3cm}|>{\centering\arraybackslash\small}p{0.6cm}|}
  ...
  & US05 & As a seller, I want to create a store with a name, description and logo so that customers can recognise my brand. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featStore} & US06 & As a seller, I want my store URL to use a readable slug so that it is easy to share. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featStore} & US07 & As a seller, I want to customise my storefront across seven sections through a WYSIWYG editor so that I can style my store live. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexVHigh}\textbf{5} \\\cline{2-5}
  \cellcolor{featStore} & US08 & As a seller, I want to apply a preset so that I can get a professional look quickly. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featStore} & US09 & As a premium seller, I want to operate multiple stores so that I can segment my offerings. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  ...
  & US10 & As a seller, I want to create a product so that I can add it to my catalogue. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featProduct} & US43 & As a seller, I want to edit an existing product so that I can keep its details accurate. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featProduct} & US44 & As a seller, I want to delete a product so that I can remove items I no longer sell. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featProduct} & US11 & As a seller, I want to upload multiple product images so that customers can see my products clearly. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featProduct} & US12 & As a seller, I want to assign tags to products so that the recommendation engine can surface them. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featProduct} & US13 & As a customer, I want to browse products by category so that I can discover relevant items. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featProduct} & US41 & As a customer, I want to see on a product page a cheaper offer for the same item and relevant alternatives across stores so that I can compare and avoid overpaying. & \cellcolor{moscowCould}\textbf{Could} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featProduct} & US42 & As a seller, I want to put an individual product on sale with a percentage discount so that I can promote single items; the larger of the product and store discount applies. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexMed}\textbf{2} \\\hline

  \cellcolor{featOrder}\makecell[c]{\textbf{F4}\\\small Order\\System}
  & US14 & As a customer, I want to add products to a cart so that I can purchase multiple items at once. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featOrder} & US15 & As a customer, I want to check out across multiple stores in a single order so that I avoid separate transactions. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featOrder} & US16 & As a customer, I want a confirmation email after ordering so that I have a record of my purchase. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featOrder} & US17 & As a seller, I want to view orders containing my products so that I can fulfil them. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  ...
  & US18 & As an admin, I want to review seller applications so that I can approve legitimate merchants. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featAdmin} & US19 & As an admin, I want to suspend or delete any store so that I can enforce marketplace rules. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featAdmin} & US20 & As an admin, I want to curate featured stores and products so that the landing page promotes quality. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  ...
  & US22 & As the system, I want to verify every new store and product via an LLM so that low-quality content is flagged automatically. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featAI} & US23 & As an admin, I want to override any AI verdict so that I remain the final authority. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featAI} & US24 & As a customer, I want an AI-verified badge on trusted products so that I can shop with confidence. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  ...
  & US25 & As a seller, I want to subscribe to a premium plan so that I can unlock advanced features. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featPremium} & US26 & As a premium seller, I want all customisation controls so that I can fully personalise my store. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  ...
  & US28 & As a developer, I want every service in a Docker container so that environments are identical. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featCICD} & US29 & As a developer, I want images built and pushed automatically on every push to main so that deployments are current. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featCICD} & US30 & As a developer, I want the droplet provisioned via Terraform so that infrastructure is reproducible. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featCICD} & US31 & As a developer, I want deployment over SSH on every push to main so that updates reach production automatically. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featCICD} & US32 & As a developer, I want secrets managed through CI secrets so that no credentials live in the repository. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  ...
  & US38 & As a user, I want to view and edit my profile so that I can keep my account details current. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featAccount} & US45 & As a user, I want to change my password so that I can keep my account secure. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
...
The product backlog comprises \textbf{45 user stories} across
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|l|>{\raggedright\arraybackslash}p{2.9cm}|X|>{\centering\arraybackslash}p{1.7cm}|}
    ...
    \multirow{2}{*}{\textbf{Release 1}}
      & Sprint 1 & Authentication and seller onboarding & US01 account creation, US02 email verification, US03 sign-in, US04 sign-out, US18 seller-application review, US37 password reset & \textasciitilde1 month \\
    \cline{2-5}
      & Sprint 2 & Store and product management & US05 store creation, US06 readable store slug, US07 storefront WYSIWYG editor, US08 style presets, US09 multiple stores (premium), US10 create product, US43 edit product, US44 delete product, US11 product images, US12 product tags, US13 category browsing, US34 store-wide sales, US42 per-product sales & \textasciitilde1.5 months \\
    \hline
    \multirow{2}{*}{\textbf{Release 2}}
      & Sprint 3 & Orders, payment, account and administration & US14 shopping cart, US15 multi-store checkout, US16 order-confirmation email, US17 seller order view, US19 store suspension/deletion, US20 featured-content curation, US21 category management, US36 secure card payment, US38 profile, US45 change password, US39 purchase history & \textasciitilde1.5 months \\
    \cline{2-5}
      & Sprint 4 & AI verification, product suggestions, premium tier, theming and DevOps & US22 AI store/product verification, US23 AI verdict override, US24 verified badge, US25 premium subscription, US26 premium customisation controls, US27 premium preservation on expiry, US28 service containerisation, US29 CI image builds, US30 Terraform provisioning, US31 SSH deployment, US32 CI-managed secrets, US33 DNS automation, US35 AI storefront redesign, US40 light/dark theming, US41 cross-store product suggestions & \textasciitilde1.5 months \\
    \hline
...
product backlog of 45 user stories across 10 features, and the planning
