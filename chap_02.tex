...
The functional requirements are grouped into nine domains. Each domain
corresponds to a backend NestJS module and to a matching set of pages on
the React frontend. Cross-cutting concerns that the product backlog tracks
as separate features, light/dark theming and the DevOps delivery chain,
are specified as non-functional requirements and detailed in
Chapter~\ref{chap:release2} rather than as functional domains.
...
\begin{itemize}[leftmargin=*,itemsep=1pt]
  ...
  \item Each product shall carry an \texttt{aiReviewed} flag set by the
        ...
        manual override.
  \item A Seller shall put an individual product on sale with a
        percentage discount and an optional end date; when a product and
        its store both have an active sale, the larger discount is
        applied (the discounts never stack).
...
\paragraph{Payments and Premium Subscription.}
\begin{itemize}[leftmargin=*,itemsep=1pt]
  \item Checkout payment shall be collected through Stripe: the server
        opens a PaymentIntent for the server-computed total and confirms
        the order only on a signature-verified webhook.
  \item A Seller shall subscribe to a recurring monthly premium tier
        through Stripe Checkout and manage or cancel it through the Stripe
        Customer Portal.
  \item Premium entitlement shall be driven by signature-verified
        subscription webhooks: access is granted on creation, preserved
        until the paid period ends on a scheduled cancellation, and revoked
        when the subscription is deleted.
  \item The Administrator shall be able to grant premium manually, without
...
\paragraph{Product Suggestions.}
\begin{itemize}[leftmargin=*,itemsep=1pt]
  \item On a product page, the system shall present cross-store
        suggestions, a cheaper ``same item'' offer when one exists and
        a set of substitute alternatives, distinct from the tag-based
        recommendations of the landing page.
  \item Suggestions shall be generated live for each product view by an
        n8n workflow (a hosted large-language model with a read-only
        database tool returning structured product identifiers), so that a
        newly listed product is eligible immediately, with no precomputed
        data to go stale.
  \item The backend shall validate every suggested identifier at serve
        time, approved, not banned, available, in stock and never the
        viewed product, with a ``better deal'' kept only when genuinely
        cheaper, and shall fall back to a deterministic catalogue query
        (same category, ranked by product-name similarity and tag overlap)
        on any workflow error, timeout or empty result, so the section is
        never empty.
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    ...
    Availability     & The production stack runs under Docker Compose with per-service restart policies that bring containers back automatically after a failure or reboot; releases are deployed by pulling the new images and recreating the affected containers. \\ \hline
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,height=0.92\textheight,keepaspectratio]{img/uc-global.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,height=0.92\textheight,keepaspectratio]{img/class-diagram.png}
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{Product Owner and Scrum Master:} Mr.\ Mohammed Aziz
        Chibani (NeuralBey), who owned the product vision, prioritised
        the backlog and ensured the Scrum framework was applied.
  \item \textbf{Development Team:} Nadim Jebali (the student),
        responsible for the entire technical realisation.
\end{itemize}
...
\begin{longtable}{|>{\centering\arraybackslash\small}p{2.2cm}|>{\centering\arraybackslash\small}p{0.9cm}|>{\small}p{7.4cm}|>{\centering\arraybackslash\small}p{1.3cm}|>{\centering\arraybackslash\small}p{0.6cm}|}
  \caption{Product backlog, user stories with MoSCoW priority and complexity}\label{tab:backlog}\\
  \hline
  \rowcolor{tableHeader}
  ...
  & US01 & As a visitor, I want to create an account with my email so that I can access the platform. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featAuth} & US02 & As a visitor, I want a verification email so that I can activate my account securely. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featAuth} & US03 & As a user, I want to sign in with my credentials so that I can access my dashboard. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featAuth} & US04 & As a user, I want to sign out so that I can protect my account on a shared device. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featAuth} & US05 & As a user, I want to reset my password by email when I forget it so that I can regain access to my account. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexMed}\textbf{2} \\\hline

  \cellcolor{featStore}\makecell[c]{\textbf{F2}\\\small Store\\Management}
  & US06 & As a seller, I want to create a store with a name, description and logo so that customers can recognise my brand. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featStore} & US07 & As a seller, I want my store URL to use a readable slug so that it is easy to share. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featStore} & US08 & As a seller, I want to customise my storefront across seven sections through a WYSIWYG editor so that I can style my store live. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexVHigh}\textbf{5} \\\cline{2-5}
  \cellcolor{featStore} & US09 & As a seller, I want to apply a preset so that I can get a professional look quickly. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featStore} & US10 & As a premium seller, I want to operate multiple stores so that I can segment my offerings. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featStore} & US11 & As a seller, I want a store-wide percentage sale so that all my products show discounted prices and a badge automatically. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexMed}\textbf{2} \\\hline

  \cellcolor{featProduct}\makecell[c]{\textbf{F3}\\\small Product\\Management}
  & US12 & As a seller, I want to create a product so that I can add it to my catalogue. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featProduct} & US13 & As a seller, I want to upload multiple product images so that customers can see my products clearly. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featProduct} & US14 & As a seller, I want to assign tags to products so that the recommendation engine can surface them. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featProduct} & US15 & As a customer, I want to browse products by category so that I can discover relevant items. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featProduct} & US16 & As a customer, I want to see on a product page a cheaper offer for the same item and relevant alternatives across stores so that I can compare and avoid overpaying. & \cellcolor{moscowCould}\textbf{Could} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featProduct} & US17 & As a seller, I want to put an individual product on sale with a percentage discount so that I can promote single items; the larger of the product and store discount applies. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featProduct} & US18 & As a seller, I want to edit an existing product so that I can keep its details accurate. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featProduct} & US19 & As a seller, I want to delete a product so that I can remove items I no longer sell. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\hline

  \cellcolor{featOrder}\makecell[c]{\textbf{F4}\\\small Order\\System}
  & US20 & As a customer, I want to add products to a cart so that I can purchase multiple items at once. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featOrder} & US21 & As a customer, I want to check out across multiple stores in a single order so that I avoid separate transactions. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featOrder} & US22 & As a customer, I want a confirmation email after ordering so that I have a record of my purchase. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featOrder} & US23 & As a seller, I want to view orders containing my products so that I can fulfil them. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featOrder} & US24 & As a customer, I want to pay securely for my order online so that my purchase is confirmed instantly. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexHigh}\textbf{3} \\\hline

  \cellcolor{featAdmin}\makecell[c]{\textbf{F5}\\\small Administration}
  & US25 & As an admin, I want to review seller applications so that I can approve legitimate merchants. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featAdmin} & US26 & As an admin, I want to suspend or delete any store so that I can enforce marketplace rules. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featAdmin} & US27 & As an admin, I want to curate featured stores and products so that the landing page promotes quality. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featAdmin} & US28 & As an admin, I want to create a category so that I can organise the catalogue. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featAdmin} & US29 & As an admin, I want to update a category so that the catalogue stays accurate. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featAdmin} & US30 & As an admin, I want to delete a category so that I can remove obsolete categories. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\hline

  \cellcolor{featAI}\makecell[c]{\textbf{F6}\\\small AI\\Verification}
  & US31 & As the system, I want to verify every new store and product via an LLM so that low-quality content is flagged automatically. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featAI} & US32 & As an admin, I want to override any AI verdict so that I remain the final authority. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featAI} & US33 & As a customer, I want an AI-verified badge on trusted products so that I can shop with confidence. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featAI} & US34 & As a seller, I want to regenerate my storefront from a natural-language prompt so that I get a professional design without manual work. & \cellcolor{moscowCould}\textbf{Could} & \cellcolor{complexHigh}\textbf{3} \\\hline

  \cellcolor{featPremium}\makecell[c]{\textbf{F7}\\\small Premium\\Tier}
  & US35 & As a seller, I want to subscribe to a premium plan so that I can unlock advanced features. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featPremium} & US36 & As a premium seller, I want all customisation controls so that I can fully personalise my store. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featPremium} & US37 & As the system, I want to preserve premium customisation after expiry so that the store keeps its design. & \cellcolor{moscowCould}\textbf{Could} & \cellcolor{complexMed}\textbf{2} \\\hline

  \cellcolor{featCICD}\makecell[c]{\textbf{F8}\\\small CI/CD \&\\Deployment}
  & US38 & As a developer, I want every service in a Docker container so that environments are identical. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featCICD} & US39 & As a developer, I want images built and pushed automatically on every push to main so that deployments are current. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featCICD} & US40 & As a developer, I want the droplet provisioned via Terraform so that infrastructure is reproducible. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featCICD} & US41 & As a developer, I want deployment over SSH on every push to main so that updates reach production automatically. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexHigh}\textbf{3} \\\cline{2-5}
  \cellcolor{featCICD} & US42 & As a developer, I want secrets managed through CI secrets so that no credentials live in the repository. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featCICD} & US43 & As a developer, I want DNS records updated automatically to the new droplet IP so that the application is always reachable. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexMed}\textbf{2} \\\hline

  \cellcolor{featAccount}\makecell[c]{\textbf{F9}\\\small Account\\Management}
  & US44 & As a user, I want to view and edit my profile so that I can keep my account details current. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\cline{2-5}
  \cellcolor{featAccount} & US45 & As a customer, I want to consult my purchase history so that I can review my past orders. & \cellcolor{moscowShould}\textbf{Should} & \cellcolor{complexLow}\textbf{1} \\\cline{2-5}
  \cellcolor{featAccount} & US46 & As a user, I want to change my password so that I can keep my account secure. & \cellcolor{moscowMust}\textbf{Must} & \cellcolor{complexMed}\textbf{2} \\\hline

  \cellcolor{featTheme}\makecell[c]{\textbf{F10}\\\small Appearance\\\& Theming}
  & US47 & As a user, I want to switch between a light and a dimmed dark theme, or follow my system preference, so that I can browse comfortably in low light. & \cellcolor{moscowCould}\textbf{Could} & \cellcolor{complexMed}\textbf{2} \\\hline

\end{longtable}

\subsection{Analysis Summary}

The product backlog comprises \textbf{47 user stories} across
...
goal, user stories and estimated duration. Each sprint runs about one to
one-and-a-half months, so the two releases together span roughly five to
six months.
...
\begin{table}[H]
  ...
  \label{tab:sprints}
  \footnotesize
  \begin{tabularx}{\textwidth}{|l|l|>{\raggedright\arraybackslash}p{2.9cm}|X|>{\centering\arraybackslash}p{1.7cm}|}
    \hline
    \rowcolor{tableHeader}
    \textcolor{white}{\textbf{Release}} & \textcolor{white}{\textbf{Sprint}} &
    \textcolor{white}{\textbf{Goal}} & \textcolor{white}{\textbf{User stories}} &
    \textcolor{white}{\textbf{Est. duration}} \\
    \hline
    \multirow{2}{*}{\textbf{Release 1}}
      & Sprint 1 & Authentication and seller onboarding & US01 account creation, US02 email verification, US03 sign-in, US04 sign-out, US25 seller-application review, US05 password reset & \textasciitilde1 month \\
    \cline{2-5}
      & Sprint 2 & Store and product management & US06 store creation, US07 readable store slug, US08 storefront WYSIWYG editor, US09 style presets, US10 multiple stores (premium), US12 create product, US18 edit product, US19 delete product, US13 product images, US14 product tags, US15 category browsing, US11 store-wide sales, US17 per-product sales & \textasciitilde1.5 months \\
    \hline
    \multirow{2}{*}{\textbf{Release 2}}
      & Sprint 3 & Orders, payment, account and administration & US20 shopping cart, US21 multi-store checkout, US22 order-confirmation email, US23 seller order view, US26 store suspension/deletion, US27 featured-content curation, US28 create category, US29 update category, US30 delete category, US24 secure card payment, US44 profile, US46 change password, US45 purchase history & \textasciitilde1.5 months \\
    \cline{2-5}
      & Sprint 4 & AI verification, product suggestions, premium tier, theming and DevOps & US31 AI store/product verification, US32 AI verdict override, US33 verified badge, US35 premium subscription, US36 premium customisation controls, US37 premium preservation on expiry, US38 service containerisation, US39 CI image builds, US40 Terraform provisioning, US41 SSH deployment, US42 CI-managed secrets, US43 DNS automation, US34 AI storefront redesign, US47 light/dark theming, US16 cross-store product suggestions & \textasciitilde1.5 months \\
    \hline
    \multicolumn{4}{|r|}{\textbf{Total estimated duration}} & \textbf{\textasciitilde5--6 months} \\
    \hline
  \end{tabularx}
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,keepaspectratio]{img/gantt.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{img/arch-3tier-v2.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,height=0.85\textheight,keepaspectratio]{img/arch-docker-v2.png}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    ...
    Product suggestions            & Hosted LLM (Google Gemini~\cite{gemini} / Groq~\cite{groq}) reasoning over the catalogue through a read-only SQL tool, returning structured product ids \\ \hline
  \end{tabularx}
\end{table}
...
actors were identified, the functional requirements grouped into eight
...
product backlog of 47 user stories across 10 features, and the planning
