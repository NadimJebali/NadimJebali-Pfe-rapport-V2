...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    Observability    & Application logs are emitted to \texttt{stdout} and aggregated by Docker; the chat assistant offers a queryable view of the database for diagnostics. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
roles; a Seller owns one or more \texttt{Store} entities, each holding
\texttt{Product} entities that are organised by \texttt{Category} and
\texttt{Tag}; orders are modelled by an \texttt{Order} aggregate and its
\texttt{OrderItem} lines, allowing a single checkout to span several
stores.
...
The product backlog comprises \textbf{35 user stories} across
\textbf{8 features}. The bulk are \textit{Must} or \textit{Should}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|l|X|>{\centering\arraybackslash}p{2.9cm}|}
    ...
    \multirow{2}{*}{\textbf{Release 1}}
      & Sprint 1 & Authentication and seller onboarding & US01--US04, US18 \\
    \cline{2-4}
      & Sprint 2 & Store and product management & US05--US13, US34 \\
    \hline
    \multirow{2}{*}{\textbf{Release 2}}
      & Sprint 3 & Orders and administration & US14--US17, US19--US21 \\
    \cline{2-4}
...
\begin{figure}[H]
  \centering
  \includegraphics[height=0.62\textheight,keepaspectratio]{img/arch-3tier.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth,keepaspectratio]{img/arch-docker.png}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lX}
    \toprule
    \textbf{Service} & \textbf{Model / provider} \\
    \midrule
    Store and product verification & Multimodal LLM (Google Gemini) screening images and listing text \\
    AI storefront redesign         & Groq \texttt{llama-3.3-70b-versatile} generating the storefront customisation \\
...
\begin{table}[H]
  ...
  \begin{tabular}{>{\centering\arraybackslash}m{1.4cm} l m{8.2cm}}
    \toprule
    \textbf{Logo} & \textbf{Tool} & \textbf{Role} \\
    \midrule
    \includegraphics[height=0.8cm]{img/github.png}    & GitHub    & Version control, pull-request review and CI/CD via GitHub Actions. \\
    \includegraphics[height=0.8cm]{img/notion.png}    & Notion    & Backlog management, task board and record of user stories. \\
    \includegraphics[height=0.8cm]{img/vscode.png}    & VS Code   & Primary IDE, extended for TypeScript, Prisma, ESLint and Git. \\
    \includegraphics[height=0.8cm]{img/docker.png}    & Docker    & Containerisation of all services for dev/prod parity. \\
    \includegraphics[height=0.8cm]{img/terraform.png} & Terraform & Infrastructure-as-code provisioning of the DigitalOcean resources. \\
    \includegraphics[height=0.8cm]{img/n8n.png}       & n8n       & Visual workflow automation orchestrating the AI features. \\
    \includegraphics[height=0.8cm]{img/postman.png}   & Postman   & Manual testing and documentation of the REST endpoints. \\
    \includegraphics[height=0.8cm]{img/pnpm.png}      & pnpm      & Fast, disk-efficient package manager for the monorepo. \\
    \bottomrule
  \end{tabular}
\end{table}
...
The platform stores its data in a \textbf{MariaDB} relational database
accessed through the \textbf{Prisma} ORM, which provides a type-safe
client and versioned migrations. The relational model captures users
and their roles, stores and their customisation, products with
categories and tags, and orders with their line items.
Figure~\ref{fig:er} shows the entity-relationship diagram.

\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth,keepaspectratio]{img/er-diagram.png}
...
Table~\ref{tab:frameworks} summarises the main frameworks and libraries.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    ...
    \hline
    TypeScript    & Statically-typed language used across the frontend and the backend. \\ \hline
    React         & Single-page application library for the customer, seller and admin interfaces. \\ \hline
    NestJS        & Modular Node.js framework structuring the backend into feature modules. \\ \hline
    Prisma        & ORM and migration tool over MariaDB. \\ \hline
    Better-Auth   & Authentication library handling sessions and email verification. \\ \hline
...
product backlog of 35 user stories across 8 features, and the planning
