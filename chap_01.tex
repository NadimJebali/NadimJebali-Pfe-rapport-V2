...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.35\columnwidth]{img/neuralbey-logo.jpeg}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\columnwidth]{img/org-neuralbey.png}
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    \toprule
    \textbf{Criterion}     & \textbf{Jumia} & \textbf{Tayara} & \textbf{Proposed Solution} \\
    \midrule
...
\begin{enumerate}[label=\textbf{D\arabic*.},leftmargin=*,itemsep=2pt]
  ...
  \item \textbf{Store-wide sale promotions} that propagate a percentage
        discount automatically across product cards, store listings and
        search results, applying the discounted price at checkout
        without per-product editing.
...
and chat assistant via n8n; store-wide percentage sales with automatic
badge propagation; AI-assisted storefront redesign; a premium tier
...
integrated payment processing; a native mobile application; advanced
fraud detection; on-premise LLM hosting; multi-currency support; and a
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{Scrum} --- organises work into time-boxed
        \emph{sprints} around three accountabilities and a small set of
        events; the most widely adopted agile framework.
  \item \textbf{Kanban} --- visualises the flow of work on a board and
        limits work-in-progress, favouring continuous flow over fixed
        iterations.
  \item \textbf{Extreme Programming (XP)} --- focuses on engineering
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\columnwidth]{img/scrum-framework.png}
...
Scrum defines three accountabilities, mapped to the present project as
follows:

\begin{itemize}[leftmargin=*,itemsep=4pt]
  \item \textbf{Product Owner} --- owns the product vision and
        prioritises the backlog. This role was held by the professional
        supervisor, \textbf{Mr.\ Mohammed Aziz Chibani} (NeuralBey),
        who also provided architectural guidance.
  \item \textbf{Scrum Master} --- ensures the framework is applied and
        removes impediments. In a single-developer context, this role
